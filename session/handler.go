...
var (
	ErrNoSessionFound  = herodot.ErrUnauthorized.WithReasonf("No valid session credentials found in the request.")
	ErrSessionInactive = herodot.ErrUnauthorized.WithReasonf("The session associated with the request is inactive.")
)
...
func (h *Handler) IsNotAuthenticated(wrap httprouter.Handle, onAuthenticated httprouter.Handle) httprouter.Handle {
	return func(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
		if _, err := h.r.SessionManager().FetchFromRequest(r.Context(), r); err != nil {
			e1 := new(ErrNoActiveSessionFound)
			e2 := new(ErrSessionIsInactive)
